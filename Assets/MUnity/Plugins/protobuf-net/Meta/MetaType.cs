...
namespace ProtoBuf.Meta
{
    ...
    public class MetaType : ISerializerProxy
    {
        ...
        private ValueMember AddField(int fieldNumber, string memberName, Type itemType, Type defaultType, object defaultValue)
        {
            ...
            MemberInfo backingField = null;
            PropertyInfo tempMi = mi as PropertyInfo;
            if (tempMi != null && tempMi.CanWrite == false)
            {
                var backingMembers = type.GetMember(string.Format("<{0}>k__BackingField", ((PropertyInfo)mi).Name), Helpers.IsEnum(type) ? BindingFlags.Static | BindingFlags.Public : BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);

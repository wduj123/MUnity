Shader "Custom/FogFullScreen" { // 全屏雾化
	Properties {
    ...
    SubShader {
        ...
        Pass {
			...
			fixed4 frag( v2f input ) : COLOR
			{
				...
				float2 screenPos = input.screenPos.xy / input.position.w;
				...
				for(int i = 0;i < circleCount;i++)
				{
					...
					for(int j = 0;j < uvCount;j++)
					{
						...
						sum += tex2D( _GrabTexture, float2(input.screenPos.x,1-input.screenPos.y) + pos) * factor / uvCount;
					}
				}
